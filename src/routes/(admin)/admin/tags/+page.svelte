<script lang="ts">
	...
	// Auto-generate slug from name for new tag
	$effect(() => {
	...
	async function addTag() {
		...
		try {
			...
			if (response.ok) {
				...
				error = errorData.message || 'Failed to create tag';
			}
		} catch (err: any) {
	...
	async function saveEdit() {
		...
		try {
			const response = await fetch(`/api/admin/tags/${editingId}`, {
				method: 'PUT',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({
					name: editName.trim(),
			...
			if (response.ok) {
				...
				const errorData = await response.json();
				error = errorData.message || 'Failed to update tag';
			}
		} catch (err: any) {
	...
	async function deleteTag(tagId: string) {
		...
		try {
			...
			if (response.ok) {
				...
				const errorData = await response.json();
				error = errorData.message || 'Failed to delete tag';
			}
		} catch (err: any) {
	...
	function getTagPostCount(tag: any): number {
		return tag.post_count || 0;
	}
</script>

<svelte:head>
	<title>Tags - EdgePress Admin</title>
</svelte:head>

<div class="tags-page">
	<div class="page-header">
		<h1>Tags</h1>
		<p class="subtitle">Tag your posts for better organization and discovery</p>
	</div>
	...
	<div class="add-form">
		...
		<div class="form-row">
			...
			<div class="form-actions">
				<button onclick={addTag} disabled={adding || !newName.trim()} class="btn-primary">
					...
					{#each tags as tag}
						<tr>
							{#if editingId === tag.id}
								...
								<td>
									<span class="badge">{getTagPostCount(tag)}</span>
								</td>
								<td>
								...
								<td colspan="4" class="delete-confirm">
									<div class="confirm-content">
										<span class="confirm-text">
											Delete "{tag.name}"?
											{#if getTagPostCount(tag) > 0}
												This will remove it from {getTagPostCount(tag)} post(s).
											{/if}
										</span>
										<div class="action-buttons">
											<button
							...
							{:else}
								...
								<td>
									<div class="action-buttons">
										<button onclick={() => startEdit(tag)} class="btn-small btn-secondary">
											Edit
										</button>
										<button onclick={() => confirmDelete(tag.id)} class="btn-small btn-danger">
											Delete
										</button>
									</div>
								</td>
