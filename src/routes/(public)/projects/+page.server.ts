...
const DEFAULT_PROJECTS_PAGE_SUBTITLE =
	...
	try {
		const [featuredProjectsRaw, standaloneProjects, pageTitle, pageSubtitle, showAllSetting] =
			await Promise.all([
				logDbCall('featured-projects', async () =>
					cache ? getFeaturedProjectsCached(db, cache, false) : getFeaturedProjects(db, false)
				),
				logDbCall('projects', () => getProjects(db)),
				logDbCall('setting:projects_page_title', () => getSetting(db, 'projects_page_title')),
				logDbCall('setting:projects_page_subtitle', () => getSetting(db, 'projects_page_subtitle')),
				logDbCall('setting:projects_page_show_all', () => getSetting(db, 'projects_page_show_all'))
			]);
		...
		if (showAll) {
			const allPostsRaw = await logDbCall('projects-category-posts', () =>
				cache ? getPublishedPostsCached(db, cache, 100, 0) : getPublishedPosts(db, 100, 0)
			);
