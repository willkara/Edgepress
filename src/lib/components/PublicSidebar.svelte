<script lang="ts">
	...
	import { Home, Search, Github, BookOpen, Folder, Mail } from 'lucide-svelte';
...
<aside class="sidebar h-full flex flex-col bg-[#050816] border-r border-white/10 text-white">
	...
	<div class="p-6 border-b border-white/10">
		...
		<div class="flex items-center gap-3">
			<div
				class="h-10 w-10 rounded-full bg-accent/20 flex items-center justify-center overflow-hidden border border-accent/30"
			>
		...
		<div class="flex items-center justify-between gap-2 mb-4">
			<div class="flex flex-col">
				...
				<span class="text-[10px] text-gray-400"
					>{user ? 'Manage profile' : 'Login to interact'}</span
				>
			</div>
			<UserMenu {user} />
		</div>
