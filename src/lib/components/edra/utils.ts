...
export const findColors = (doc: PMNode): DecorationSet => {
	...
	doc.descendants((node: PMNode, position: number): void => {
		...
		Array.from(node.text.matchAll(hexColor)).forEach((match: RegExpMatchArray) => {
			const color = match[0];
 ...
 */
export const duplicateContent = (editor: Editor, node: PMNode): void => {
	const { view } = editor;
	const { state } = view;
