...
export function buildEmbeddingInput(payload: PostVectorPayload): string {
	...
	return raw.length > MAX_EMBEDDING_INPUT_LENGTH ? raw.slice(0, MAX_EMBEDDING_INPUT_LENGTH) : raw;
}
...
export async function searchPosts(
	...
	const matches = (
		response as { matches?: Array<{ id: string; score: number; metadata?: PostVectorMetadata }> }
	).matches;
	if (!matches) {
		return [];
	}
